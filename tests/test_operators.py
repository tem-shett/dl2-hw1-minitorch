...
@pytest.mark.task0_2
@given(
    floats(min_value=-10, max_value=10, allow_nan=False),
    floats(min_value=1e-2, max_value=100, allow_nan=False),
)
def test_sigmoid(a: float, d: float) -> None:
...
def test_other(x, dx, d) -> None:
    """
    Write a test that ensures some other property holds for your functions.
    Other property: (1/x)' is less than 0 and strictly increasing for x > 0
    """
    assert inv_back(x, d) < 0
    assert inv_back(x, d) < inv_back(x + dx, d)
